...
import { getAllPackageList, getLifeRewardById, postLifeReward, updateLifeReward } from '../../../api/login/Login';
...
function AddLifeReward() {
    ...
    const handleFileChange = (e) => {
        ...
        const fileData = files.map((file) => {
            // Optionally convert to Base64 or use URL.createObjectURL
            return URL.createObjectURL(file); // This generates a temporary URL
        });

        setFormData({
            ...formData,
            imgs: fileData, // Store the array of strings
        });
        setErrors({
            ...errors,
            imgs: "", // Reset errors if any
        });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(validationErrors).length > 0) {
            ...
            if (params?.id) {
                ...
                try {
                    const res = await postLifeReward(formData);
                    if (res?.statusCode == '200') {
                        setLoad(false)
